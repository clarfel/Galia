...
class Movies extends Component {
  ...
  loadMoreSection() {
    axios
      .all([
        axios.get(
          `${config.API_URL}/list_movies.json?genre=action&sort_by=rating&order_by=desc&limit=${config.RESULT_SIZE}`
        ),
        axios.get(
          `${config.API_URL}/list_movies.json?genre=adventure&sort_by=rating&order_by=desc&limit=${config.RESULT_SIZE}`
        ),
      ])
      .then(
  ...
  scroll(e) {
    ...
    if (
      nevent.contentSize.height <
        nevent.contentOffset.y + nevent.layoutMeasurement.height + 250 &&
  ...
  componentDidMount() {
    axios
      .all([
        axios.get(
          `${config.API_URL}/list_movies.json?sort_by=like_count&order_by=desc&limit=10`
        ),
        axios.get(
          `${config.API_URL}/list_movies.json?sort_by=rating&order_by=desc&limit=${config.RESULT_SIZE}`
        ),
        axios.get(
          `${config.API_URL}/list_movies.json?sort_by=year&order_by=desc&limit=${config.RESULT_SIZE}`
        ),
      ])
      .then(
        axios.spread((featured, popular, recent) => {
          this.setState({
      ...
      <View style={style.main}>
        ...
        {!this.state.init && (
          ...
          >
            ...
            />
            <Text style={[style.textCenter, style.loadingText]}>
              Loading...
            </Text>
          </View>
        )}
            ...
            >
              {this.state.featured.map((el, index) => (
                <TouchableOpacity
                  ...
                  key={index}
                  onPress={() => {
                    this.props.navigation.navigate("Details", { data: el });
                  }}
                >
                  <Image
                    source={{ uri: el.background_image }}
                    style={[movieDetails.moviePoster]}
                    resizeMode="cover"
                  />
                  <LinearGradient
                  ...
                  >
                    ...
                    >
                      <Image
                        source={{
                          uri: el.medium_cover_image || el.large_cover_image,
                        }}
                    ...
                    <View style={[style.flexbox]}>
                      ...
                      <Text
                        style={[
                          movie.subText,
                          movieDetails.leftWidth,
                          { marginBottom: 5, fontWeight: "500" },
                        ]}
                        numberOfLines={1}
                        ellipsizeMode="tail"
                      >{`${el.runtime} mins | ${el.mpa_rating || "-"} | ${
                        el.language
                      }`}</Text>
                      <View
                        style={[
            ...
            <View style={[style.flexboxContainer, style.flexColumn]}>
              ...
              >
                ...
                >
                  ...
                  <TouchableOpacity
                    style={[style.flexbox, style.relative]}
                    onPress={() => {
                  ...
                  <View style={[style.flexboxContainer, { paddingRight: 15 }]}>
                    {config.GENRE.map((el, index) => (
                      ...
                      >
                        <Image
                          source={{
                            uri: `${config.GENRE_IMAGE_URL}${el.shortName}.jpg`,
                          }}
                          style={[movie.genreImage]}
                          resizeMode="cover"
                        />
                        <LinearGradient
                          colors={[
                        ...
                        >
                          ...
                          >
                            <Text
                              style={[movie.title, movie.genreTitle]}
                              numberOfLines={1}
                            >
                              {el.name}
                            </Text>
                ...
                <View style={{ marginBottom: 50 }}>
                  ...
                  <View style={[style.flexbox, style.border, style.sectionBox]}>
                    ...
                    >
                      ...
                      >
                        <Text style={[style.textRight, movie.seeAll]}>
                          See All
                        </Text>
                        <View style={[style.border, movie.seeAllArrow]}></View>
                      </TouchableOpacity>
                    ...
                    <ScrollView
                      horizontal={true}
                      style={style.bothsideOverFlow}
                    >
                      <View
                        style={[style.flexboxContainer, { paddingRight: 15 }]}
                      >
